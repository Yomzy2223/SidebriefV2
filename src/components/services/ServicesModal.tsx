...
export const ServicesModal = ({
  ...
  const handleRemoveService = (serviceName: string) => {
    const removedService = OtherServices.find(
      (service) => service.name === serviceName
    );
  ...
  return (
    <Modal show={open} onClose={close} size={"5xl"}>
      <Modal.Header>
        Other services you can get alongside your business registration
      </Modal.Header>
      <Modal.Body>
        ...
        <div className="flex justify-between mt-8">
          <Button onClick={close} color="secondary" className="px-12 py-3">
          ...
          <Button
            onClick={close}
            color="gray"
            className="px-12 py-3 bg-white border-none"
          >
            <span className="mx-3 text-primary text-lg">See All Services</span>
          </Button>
        </div>
      </Modal.Body>
    </Modal>
